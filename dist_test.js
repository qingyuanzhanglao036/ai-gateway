...
var MODEL_COOLDOWN_DURATION_MS = 10 * 60 * 1e3;
var PROBE_DEBOUNCE_INTERVAL_MS = 5 * 1e3;
var MAX_REFILL_PROBE_ROUNDS = 3;
var MAX_REFILL_PER_ROUND = 2;
var MODEL_MAX_PROBE_FAILURES = 3;
var MAX_SESSION_STICKINESS_HISTORY = 5;
var SESSION_STICKINESS_TTL_SECONDS = 60 * 60;
var KV_KEYS = {
  PROVIDERS: "providers",
  PROXY_KEYS: "proxy:keys",
  TIERS: "tier:pools",
  // 三大梯队持久化键
  PROBE_AUDITION_CURSOR: "probe:audition:cursor",
  // 海选探测轮询游标持久化键
  PROBE_AUDITION_LOGS: "probe:audition:logs",
  // 海选探测记录（隔离存储，不作为业务淘汰依据）
  PROBE_OPENCLAW_LOGS: "probe:openclaw:logs",
  // OpenClaw专属探测记录（隔离存储）
  BUSINESS_LATENCY_PREFIX: "stats:latency:model:",
  // 真实业务延迟样本（前缀，每个模型最近50条）
  SESSION_STICKINESS_PREFIX: "session:sticky:",
  // 会话粘性调度记录前缀
...
// src/storage.ts
var stickinessMemoryCache = {};
async function getSessionStickiness(env, sessionId) {
...
async function recordSessionSuccessModel(env, sessionId, providerId, modelId) {
  ...
  if (top && top.providerId === providerId && top.modelId === modelId && now - top.lastSuccessAt < 6e4) {
    return;
  }
...
async function getAuditionCursor(env) {
...
async function saveAuditionCursor(env, cursor) {
  if (!env?.KV) return;
  try {
...
async function getProbeLogs(env, type) {
  if (!env?.KV) return [];
  try {
...
async function recordProbeLog(env, result) {
  if (!env?.KV) return;
  try {
...
var MAX_BUSINESS_LATENCY_SAMPLES = 50;
async function getModelBusinessLatency(env, providerId, modelId) {
  const fallback = {
    providerId,
    modelId,
    samples: [],
...
async function recordBusinessLatency(env, providerId, modelId, sample) {
  if (!env?.KV) return;
  try {
    const key = `${KV_KEYS.BUSINESS_LATENCY_PREFIX}${providerId}:${modelId}`;
...
async function getTierConfig(env) {
...
async function setTierConfig(env, config) {
  if (!env?.KV) return;
  try {
...
async function adminAuthMiddleware(c, next) {
  ...
  if (!sessionId) {
    ...
    sessionId = url.searchParams.get("session_id") || c.req.header("x-session-id") || void 0;
  }
  if (!sessionId) {
    const url = new URL(c.req.url);
...
var globalTimerHandle = null;
var lastFlushTimestamp = Date.now();
var pendingHealthCache = {};
...
async function queueHealthUpdate(env, providerId, health) {
  if (debugConfig.debugMode) {
...
function ensureGlobalTimer(env) {
  ...
  }, debugConfig.flushIntervalSec * 1e3);
}
...
async function writeHealthDirect(env, providerId, health) {
  if (!env?.KV) return;
  try {
...
async function flushPendingHealthCache(env) {
  clearGlobalTimer();
...
async function checkAndFlushOnRequest(env) {
  if (debugConfig.debugMode) return;
  const elapsed = Date.now() - lastFlushTimestamp;
  if (elapsed >= debugConfig.flushIntervalSec * 1e3 || pendingItemsCount >= debugConfig.maxCacheItems) {
    await flushPendingHealthCache(env);
  }
}
async function updateDebugConfig(env, updates) {
  const oldMode = debugConfig.debugMode;
  const newMode = updates.debugMode !== void 0 ? updates.debugMode : oldMode;
  if (oldMode !== newMode) {
    await flushPendingHealthCache(env);
    if (newMode) {
      clearGlobalTimer();
...
// src/probe.ts
var lastTierProbeTime = {
  tier1: 0,
  tier2: 0,
  tier3: 0
};
function getFlatModelList(providers) {
  const list = [];
...
function buildProbeHeaders(apiKey, apiType) {
...
async function probeSingleModel(env, provider, model, isExclusiveOpenClaw = false) {
...
async function runAuditionProbeRound(env) {
  const providers = await getProviders(env);
  ...
  const lastIndex = flatModels.findIndex(
    (item) => item.provider.id === providers[savedCursor.providerIndex]?.id && item.model.id === savedCursor.lastModelId
  );
  if (lastIndex >= 0) {
...
async function runOpenClawProbe(env, targetProviderId, targetModelId) {
  const providers = await getProviders(env);
  ...
  return {
    success: probeRes.success,
    message: probeRes.success ? `\u6A21\u578B\u3010${candidateModel.id}\u3011\u901A\u8FC7 OpenClaw \u4E13\u5C5E\u6D4B\u8BD5\uFF0C\u5DF2\u6210\u529F\u8D4B\u4E88 openclaw \u6807\u7B7E` : `\u6A21\u578B\u3010${candidateModel.id}\u3011\u672A\u901A\u8FC7 OpenClaw \u4E13\u5C5E\u6D4B\u8BD5: ${probeRes.message}`,
    probed: result,
    assignedTag
  };
}
async function triggerTierRefill(env, tierKey) {
  const now = Date.now();
  ...
  const existingKeys = new Set(
    (targetTier.models || []).map((m) => `${m.providerId}:::${m.modelId}`)
  );
  ...
  while (round < MAX_REFILL_PROBE_ROUNDS && targetTier.models.length < targetTier.maxSeats && candidateIndex < candidates.length) {
    round++;
    const roundProbeResults = [];
    ...
    for (let i = 0; i < batchSize; i++) {
      ...
      if (probeRes.success) {
        ...
        if (isAuthOrNotFound || cand.model.failCount >= MODEL_MAX_PROBE_FAILURES) {
          ...
          cand.model.deadReason = isAuthOrNotFound ? `\u63A2\u6D4B\u9274\u6743\u6216\u7AEF\u70B9\u5931\u6548 (HTTP ${probeRes.statusCode})` : `\u7D2F\u8BA1\u63A2\u6D4B\u5931\u8D25 ${cand.model.failCount} \u6B21\u7194\u65AD`;
        } else {
    ...
    const successfulCandidates = roundProbeResults.filter((r) => r.success).sort((a, b) => a.latencyMs - b.latencyMs);
...
function extractSessionId(c, body, rawKey) {
  const headerSessionId = c.req.header("x-session-id") || c.req.header("x-conversation-id") || c.req.header("session-id") || c.req.header("conversation-id");
...
async function resolveScheduledModel(env, requestedModel, sessionId, providers, tiers) {
  const lower = requestedModel.toLowerCase();
  let targetTierKey = null;
...
async function handleBusinessModelFailure(env, providerId, modelId, statusCode, failReason) {
  const now = Date.now();
  const [providers, tiers] = await Promise.all([
    getProviders(env),
    getTierConfig(env)
  ...
  if (targetProvider && targetProvider.models) {
    ...
    if (targetModel) {
      ...
      if (isAuthOrNotFound || targetModel.failCount >= MODEL_MAX_PROBE_FAILURES) {
        ...
        targetModel.deadReason = isAuthOrNotFound ? `\u771F\u5B9E\u4E1A\u52A1\u8BF7\u6C42\u9274\u6743\u6216\u7AEF\u70B9\u5931\u6548 (HTTP ${statusCode})` : `\u771F\u5B9E\u4E1A\u52A1\u7D2F\u8BA1\u5931\u8D25 ${targetModel.failCount} \u6B21\u7194\u65AD`;
      } else {
  ...
  for (const tKey of tierKeys) {
    ...
    if (t && t.models && Array.isArray(t.models)) {
      ...
      t.models = t.models.filter((m) => !(m.providerId === providerId && m.modelId === modelId));
...
async function handleProxy(c) {
  ...
  const finishAndLog = async (statusCode, failReason = "-") => {
    recordLog({
...
async function handleBatchSave(c) {
  try {
    const body = await c.req.json();
...
async function handleGetLogs(c) {
  const logs = getMemoryLogs();
  const debugConfig2 = getDebugConfig();
  return c.json({
    success: true,
    data: {
      logs,
...
async function handleClearLogs(c) {
  clearMemoryLogs();
  return c.json({
    success: true,
    message: "\u5185\u5B58\u65E5\u5FD7\u5DF2\u6E05\u7A7A"
...
async function handleUpdateDebugConfig(c) {
  try {
    const body = await c.req.json();
    ...
    return c.json({
      success: true,
      data: updated,
      message: updated.debugMode ? "\u5DF2\u5F00\u542F\u8C03\u8BD5\u6A21\u5F0F\uFF08\u4EC5\u8BB0\u5F55\u62A5\u9519\u4E0E\u8D85\u65F6\uFF0C\u5185\u5B58\u672A\u843D\u5730\u7F13\u5B58\u5DF2\u7EDF\u4E00\u843D\u76D8\uFF09" : "\u5DF2\u5207\u6362\u4E3A\u6B63\u5F0F\u6A21\u5F0F\uFF08\u5DF2\u542F\u7528\u5185\u5B58\u7F13\u5B58\u4E0E 30 \u79D2\u5B9A\u65F6\u843D\u76D8\uFF09"
    });
  } catch (err) {
    const error = err;
    return c.json({
...
async function handleFetchUpstreamModels(c) {
  try {
    const body = await c.req.json();
    ...
    if (providerId === "opencode" || isOpenCodeProvider(providerId || "")) {
      ...
      const resPayload = testRes.data;
      const list = resPayload?.data || [];
      const modelIds = Array.from(new Set(list.map((m) => String(m.id || "")).filter(Boolean)));
      return c.json({
        success: true,
    ...
    try {
      ...
      return c.json({
        success: false,
        message: isAbort ? "\u62C9\u53D6\u4E0A\u6E38\u6A21\u578B\u8BF7\u6C42\u8D85\u65F6 (10\u79D2)" : `\u8FDE\u63A5\u4E0A\u6E38\u5931\u8D25: ${fetchErr.message}`
      }, 502);
    }
  } catch (err) {
    return c.json({
      success: false,
      message: `\u5904\u7406\u8BF7\u6C42\u5931\u8D25: ${err.message}`
    }, 500);
  }
}
async function handleTriggerTierRefill(c) {
  try {
    const body = await c.req.json();
    if (!body.tierKey || !["tier1", "tier2", "tier3"].includes(body.tierKey)) {
      return c.json({ success: false, message: "\u65E0\u6548\u7684 tierKey \u53C2\u6570\uFF0C\u4EC5\u652F\u6301 tier1 / tier2 / tier3" }, 400);
    }
    ...
    return c.json({
      success: false,
      message: `\u68AF\u961F\u8865\u4F4D\u6267\u884C\u5F02\u5E38: ${err.message}`
    }, 500);
  }
}
async function handleRunAuditionProbe(c) {
  try {
    ...
    return c.json({
      success: false,
      message: `\u6D77\u9009\u63A2\u6D4B\u6267\u884C\u5F02\u5E38: ${err.message}`
    }, 500);
  }
}
async function handleRunOpenClawProbe(c) {
  try {
    ...
    return c.json({
      success: false,
      message: `OpenClaw \u4E13\u5C5E\u63A2\u6D4B\u5F02\u5E38: ${err.message}`
    }, 500);
  }
}
async function handleGetProbeStatus(c) {
  try {
...
/* \u4E09\u5927\u68AF\u961F\u6C60\u540E\u53F0\u7BA1\u7406\u4E0E\u524D\u53F0\u5C55\u793A\u6837\u5F0F */
.tier-pools-grid { display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: var(--space-md); margin-block-start: var(--space-md); }
...
async function renderHomePage(c, isLoggedIn) {
  ...
  const renderTierPoolSection = (tier, icon, badgeLabel) => {
  ...
  const sampleModel = tierConfig.tier1.models.length > 0 ? `${tierConfig.tier1.models[0].providerId}/${tierConfig.tier1.models[0].modelId}` : "opencode/deepseek-v4-flash-free";
...
function renderProxyKeyCard(k) {
  ...
  article.innerHTML = '<div class="key-main"><span class="key-icon" aria-hidden="true"><i class="fas fa-key"></i></span>' +
    ...
    '<div class="key-meta"><h3>' + escapeHtml(k.name) + '</h3><span class="key-meta__sep">-</span><p>\u521B\u5EFA\u4E8E ' + new Date(k.createdAt).toLocaleDateString() + ' \xB7 ' + expStr + '</p></div></div></div>' +
...
app.get("/", async (c) => {
  ...
  const sessionId = getCookie2(c, "session_id") || c.req.query("session_id");
